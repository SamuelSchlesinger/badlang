...
runtimeSource = unlines
  ...
  , "Value* make_closure(Value* (*fn_ptr)(Value*, Value*), Value* env) {"
  , "    Value* v = (Value*)stele_malloc(sizeof(Value));"
  , "    v->tag = TAG_CLOSURE;"
  , "    v->refcount = 1;"
  ...
  , "}"
  , ""
  , "Value* fn_file_exists(Value* arg) {"
  , "    Value* pathVal = record_field(arg, \"path\");"
  , "    if (!pathVal || pathVal->tag != TAG_STR) {"
  ...
  , "}"
  , ""
  , "Value* checked_div(Value* a, Value* b) {"
  ...
  , "}"
  , ""
  , "Value* checked_mod(Value* a, Value* b) {"
