...
-- divine expressions, and let-in chains are lowered to flat instructions
...
-- * __No SSA phi nodes__: Join points (divine results) use a pre-declared
...
data IRDecl
  = IRFunc !String IRFuncBody    -- ^ A rite compiled to IR
  | IRMain IRFuncBody            -- ^ The ritual main
...
data Instr
  ...
  -- Calls
  | ICall      Var String Var           -- ^ @var = rite_name(arg)@
  -- Reference counting
  ...
  -- IO
  | IUtter     Var                      -- ^ @utter(var)@
  | IWhisper   Var                      -- ^ @whisper(var)@
  | IHearken   Var                      -- ^ @var = runtime_hearken()@
  | IScry      Var                      -- ^ @var = runtime_scry()@
  -- Plumbing
