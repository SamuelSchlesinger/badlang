...
| Category | Types |
|----------|-------|
| Keywords | `rite`, `ritual`, `altar`, `given`, `seal`, `let`, `invoke`, `summon`, `divine`, `hearken`, `scry`, `utter`, `whisper` |
| Identifiers | `ident` |
| Literals | `int`, `str` |
| Operators | `+`, `-`, `*`, `/`, `%`, `==`, `!=`, `<`, `>`, `<=`, `>=`, `&&`, `\|\|` |
| Delimiters | `{\|`, `\|}`, `(`, `)`, `.`, `,`, `:`, `=`, `=>` |
...
3. **Letters or `_`** → `lex_word`: Scan to the end of the alphanumeric
   ...
   the token type is the keyword itself (e.g., `"rite"`). A lone `_` becomes
...
The `skip_ws_cmt` rite handles both whitespace and line comments recursively:
